...
export default function DashboardPage() {
  ...
  const { data: dashboard, isLoading } = useQuery({
    queryKey: ['dashboard', user?.id, user?.role, user?.agencyId, user?.brokerId],
    queryFn: () => dashboardAPI.getDashboard(),
    enabled: canViewDashboard,
  })

  const { data: dueDates, isLoading: dueDatesLoading } = useQuery({
    queryKey: ['due-dates', user?.id, user?.role, user?.agencyId, user?.brokerId],
  ...
  // Mock data for charts (will be replaced with real data when backend provides it)
  ...
  const propertyStatusPieColors = ['#64748b', '#ef4444', '#22c55e', '#fbbf24'] // Vago, Vencido, Em dia, Pendente
  ...
  const revenueData = isCEO
    ? [
        {
          name: 'Recebido',
          value: overview.receivedRevenue ?? 0,
        },
        {
          name: 'Pendente',
          value: (overview.monthlyRevenue ?? 0) - (overview.receivedRevenue ?? 0) - (overview.overdueRevenue ?? 0),
        },
        {
          name: 'Vencido',
          value: overview.overdueRevenue ?? 0,
        },
      ]
    : [
        {
          name: 'Recebido',
          value: overview.receivedValue ?? 0,
        },
        {
          name: 'Pendente',
          value: (overview.monthlyRevenue ?? 0) - (overview.receivedValue ?? 0) - (overview.overdueValue ?? 0),
        },
        {
          name: 'Vencido',
          value: overview.overdueValue ?? 0,
        },
      ]
  ...
  const propertyStatusData = isCEO && dashboard?.propertyStatus
    ? [
        { name: 'Disponíveis', value: dashboard.propertyStatus.available ?? 0 },
        { name: 'Ocupados', value: dashboard.propertyStatus.occupied ?? 0 },
        { name: 'Vencidos', value: dashboard.propertyStatus.overdue ?? 0 },
        { name: 'Manutenção', value: dashboard.propertyStatus.maintenance ?? 0 },
      ]
    : [
        { name: 'Vagos', value: overview.vacantUnits ?? 0 },
        { name: 'Vencidos', value: overview.overdueUnits ?? 0 },
        { name: 'Em dia', value: overview.onTimeUnits ?? 0 },
        { name: 'Pendentes', value: overview.pendingUnits ?? 0 },
      ]
    ...
    <div className="space-y-6 sm:space-y-8">
          ...
          <>
            ...
            {canViewProperties && (
              <StatCard
                ...
                value={overview.occupiedProperties || 0}
                icon={Users}
                color="green"
              />
            )}
      ...
      {isCEO && (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
          ...
          <StatCard
            title="Inadimplência"
            value={`${overview.defaultRate || 0}%`}
            icon={AlertCircle}
            color={overview.defaultRate > 10 ? 'red' : overview.defaultRate > 5 ? 'yellow' : 'green'}
          />
          <StatCard
                  ...
                  <PieChart>
                    ...
                    >
                      {propertyStatusData.map((entry, index) => (
                        <Cell key={`cell-status-${index}`} fill={propertyStatusPieColors[index % propertyStatusPieColors.length]} />
                      ))}
                    </Pie>
                    <Legend />
