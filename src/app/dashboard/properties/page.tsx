...
import { propertiesAPI, usersAPI, addressAPI } from '@/lib/api'
...
export default function PropertiesPage() {
  ...
  const [newProperty, setNewProperty] = useState({
    ...
    dueDay: '',
    ownerId: '',
    agencyFee: '', // Property-specific agency fee (only for AGENCY_MANAGER)
  })
  ...
  const closeAllModals = () => {
    ...
    setPropertyToAssignTenant(null)
    setSelectedTenantId('none')
    setPropertyToDelete(null) // Clear delete modal state
    setExistingImageCount(0) // Reset existing image count
    setImageRefreshTrigger(prev => prev + 1) // Trigger image refresh
    setDeleting(false)
  }
  ...
  const deletePropertyMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ['dashboard'] })
      queryClient.invalidateQueries({ queryKey: ['due-dates'] })
      closeAllModals() // Close all modals including delete modal
  ...
  const handleUpdateProperty = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const propertyToSend = {
        ...
        status: selectedProperty.status || 'DISPONIVEL', // Keep existing status or default
      ...
      if (editSelectedImages.length > 0) {
        ...
        try {
          ...
          setEditSelectedImages([]) // Clear selected images
      ...
      <div className="relative w-full">
        {/* Main Image */}
          <div className="relative w-full h-64 bg-gray-100 rounded-md overflow-hidden">
            <img
              src={`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8081'}/properties/${propertyId}/image/public?imageId=${images[currentIndex].id}`}
        ...
        {images.length > 1 && (
          <div className="flex gap-2 mt-3 overflow-x-auto">
            {images.map((image, index) => (
              ...
              >
                <img
                  src={`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8081'}/properties/${propertyId}/image/public?imageId=${image.id}`}
    ...
    useEffect(() => {
      const fetchImages = async () => {
        try {
          ...
          const data = await propertiesAPI.getPropertyImages(propertyId)
          setImages(data)
          // Notify parent component of image count
                    ...
                    <Button
                      ...
                      onClick={async () => {
                        try {
                          ...
                          // Refresh properties list to update property cards
                          if (onImageDeleted) {
                            onImageDeleted()
                          }
  ...
  // Property images component for displaying all images
  const PropertyImages = ({ propertyId, propertyName }: { propertyId: string, propertyName?: string }) => {
    const [images, setImages] = useState<any[]>([])
    const [loading, setLoading] = useState(true)
    ...
    if (loading) {
      return (
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
          {[1, 2, 3].map(i => (
            <div key={i} className="w-full h-32 bg-gray-100 rounded-md flex items-center justify-center animate-pulse">
    ...
    return (
      <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
        {images.map((image, index) => (
          <div key={image.id} className="relative w-full h-32 rounded-md overflow-hidden">
            <img
              src={`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8081'}/properties/${propertyId}/image/public?imageId=${image.id}`}
              alt={`${propertyName || 'Imóvel'} - Imagem ${index + 1}`}
  ...
  const PropertyImage = ({ propertyId, propertyName }: { propertyId: string, propertyName?: string }) => {
    ...
    // Reset error state when propertyId or refresh trigger changes
    useEffect(() => {
      setErrored(false)
    ...
    const imageUrl = `${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8081'}/properties/${propertyId}/image/public?t=${imageRefreshTrigger}`;
