...
export default function AgenciesPage() {
  ...
  // Check permissions - CEO can view, edit, and delete agencies
    ...
    return (
      <div className="flex items-center justify-center min-h-[400px]">
        <div className="text-center">
          ...
          <p className="text-muted-foreground">You do not have permission to view agencies.</p>
        </div>
      </div>
    )
  }
  ...
  const updateAgencyMutation = useMutation({
    ...
    onError: (error: any) => {
      let errorMessage = 'Erro ao atualizar agência'
      if (error?.data) {
        const data = error.data
        if (data.errors && Array.isArray(data.errors)) {
          errorMessage = data.errors.map((e: any) => e.message || (e.path ? e.path.join('.') : '')).join(', ')
  ...
  const deleteAgencyMutation = useMutation({
    ...
    onError: (error: any) => {
      let errorMessage = 'Erro ao excluir agência'
  ...
  const handleEditAgency = async () => {
    if (!canUpdateAgencies) {
      toast.error('Você não tem permissão para atualizar agências')
      return
    }
    ...
    if (selectedAgency.zipCode && selectedAgency.zipCode.trim() !== '') {
      if (!isValidCEPFormat(selectedAgency.zipCode)) {
        toast.error('CEP inválido. Por favor, insira um CEP válido (00000-000)')
        setUpdating(false)
        return
      }
    }
  ...
  const handleDeleteAgency = async () => {
    if (!canDeleteAgencies) {
      toast.error('Você não tem permissão para excluir agências')
      return
    }
    ...
    <TooltipProvider>
      <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold text-foreground">Agências</h1>
            <p className="text-muted-foreground">Visualize e gerencie agências imobiliárias cadastradas</p>
          </div>
        </div>
        ...
        {isLoading ? (
          <div className="text-center py-12">
            ...
            <p className="text-muted-foreground mt-2">Carregando agências...</p>
          </div>
          ...
          <div className="text-center py-12 bg-card border border-border rounded-lg">
            ...
            <p className="text-muted-foreground mb-4">
              Nenhuma agência encontrada no sistema
            </p>
          </div>
        )}
        ...
        <Dialog open={showDetailModal} onOpenChange={setShowDetailModal}>
          <DialogContent className="max-w-xl">
            <DialogHeader>
              ...
              <DialogDescription>
                Visualize todas as informações da agência
              </DialogDescription>
            </DialogHeader>
            {agencyDetail && (
        ...
        <Dialog open={showEditModal} onOpenChange={setShowEditModal}>
          <DialogContent className="max-w-2xl">
            <DialogHeader>
              ...
              <DialogDescription>
                Atualize as informações da agência. Todos os campos são opcionais, exceto nome e e-mail.
              </DialogDescription>
            </DialogHeader>
            {selectedAgency && (
              <div className="space-y-4">
                ...
                <div className="flex justify-end gap-2 pt-2">
                  ...
                  <Button onClick={handleEditAgency} disabled={updating} className="bg-primary hover:bg-primary/90 text-primary-foreground">
        ...
        <AlertDialog open={!!agencyToDelete} onOpenChange={() => setAgencyToDelete(null)}>
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                Tem certeza que deseja excluir a agência "{agencyToDelete?.name}"? 
