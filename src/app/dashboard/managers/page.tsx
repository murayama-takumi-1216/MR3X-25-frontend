...
export default function ManagersPage() {
    ...
    return (
      <div className="flex items-center justify-center min-h-[400px]">
        <div className="text-center">
          ...
          <p className="text-muted-foreground">Você não tem permissão para visualizar gerentes.</p>
        </div>
      </div>
    )
  }
  ...
  const { data: managers, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      console.log('[ManagersPage] Raw API response:', list.items?.map((m: any) => ({
        name: m.name,
        email: m.email,
        agencyId: m.agencyId,
      })))
      console.log('[ManagersPage] Current user agencyId:', user?.agencyId)
      
      // Filter by agencyId on frontend as additional safety measure
      // Only show managers from the current agency admin's agency
      const filtered = (list.items || []).filter((manager: any) => {
        // If current user has agencyId, only show managers with matching agencyId
        if (user?.agencyId) {
          ...
          const isMatch = managerAgencyId === userAgencyId
          console.log(`[ManagersPage] Manager ${manager.name}: agencyId=${managerAgencyId}, match=${isMatch}`)
          return isMatch
        }
        // If user doesn't have agencyId, show all managers (they haven't created an agency yet)
        console.log(`[ManagersPage] User has no agencyId, showing all managers`)
        return true
      })
      console.log('[ManagersPage] Filtered managers:', filtered.length, 'out of', list.items?.length)
      return filtered
    },
    enabled: canViewUsers, // Allow query to run regardless of agencyId
    staleTime: 0,
  ...
  const createManagerMutation = useMutation({
    mutationFn: (data: any) => usersAPI.createUser({
      ...
      // agencyId will be automatically set by backend based on AGENCY_ADMIN's agencyId
    }),
    onSuccess: () => {
    ...
    onError: (error: any) => {
      ...
      if (error.message) {
        ...
        if (message.includes('already exists') || message.includes('user already exists') || message.includes('email already exists')) {
          ...
          errorMessage = 'Sem permissão para criar gerente.'
        } else {
  ...
  const updateManagerMutation = useMutation({
    ...
    onError: (error: any) => {
      ...
      // Extract specific error message
      let errorMessage = 'Erro ao atualizar gerente'
      ...
      if (error.message) {
        ...
        if (message.includes('already exists') || message.includes('email already exists')) {
          ...
          errorMessage = 'Sem permissão para atualizar gerente.'
        } else {
          errorMessage = error.message
        }
      }
  ...
  const handleCreateManager = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const docResult = enable2026 ? validateDocument2026(newManager.document) : validateDocument(newManager.document)
      ...
      // Default password for manager first login
    ...
    <TooltipProvider>
      <div className="space-y-6">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            <h1 className="text-2xl sm:text-3xl font-bold">Gerentees</h1>
            <p className="text-sm sm:text-base text-muted-foreground mt-1">
              Gerencie todos os gerentees da sua agência
            </p>
          </div>
          <div className="flex items-center gap-2 w-full sm:w-auto">
            ...
            {canCreateUsers && (
              <Tooltip>
                ...
                <TooltipContent>Cadastrar Gerente</TooltipContent>
              </Tooltip>
            )}
          </div>
        </div>
          ...
          viewMode === 'table' ? (
                  ...
                  <div key={manager.id} className="border-b border-border last:border-b-0 p-4">
                    <div className="flex items-start justify-between mb-3 min-w-0 gap-2">
                      <div className="flex-1 min-w-0">
                        <h3 className="font-semibold text-lg truncate" title={manager.name || 'Sem nome'}>{manager.name || 'Sem nome'}</h3>
                        <p className="text-sm text-muted-foreground truncate" title={manager.email || '-'}>{manager.email || '-'}</p>
                      </div>
                      <Badge className="bg-blue-500 text-white text-xs flex-shrink-0">Gerente</Badge>
                    </div>
                    ...
                    <div className="space-y-2 mb-4 min-w-0">
                      ...
                      {manager.address && (
                        <div className="flex items-start text-sm text-muted-foreground min-w-0">
                          ...
                          <span className="truncate" title={`${manager.address}${manager.city ? `, ${manager.city}` : ''}${manager.state ? ` - ${manager.state}` : ''}`}>
                            {manager.address}
                            {manager.city && `, ${manager.city}`}
                            {manager.state && ` - ${manager.state}`}
                          </span>
            ...
            <div className="flex justify-center w-full">
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 w-full max-w-7xl px-2 items-stretch justify-center">
                {managers.map((manager: any) => (
                  <Card key={manager.id} className="transition-all hover:shadow-md flex flex-col w-full max-w-[400px] mx-auto overflow-hidden">
                      ...
                      <div className="flex h-full min-w-0">
                        ...
                        <div className="flex-1 flex flex-col justify-between p-4 min-w-0 overflow-hidden">
                          <div className="min-w-0 space-y-1">
                            <h3 className="text-lg font-bold truncate" title={manager.name || 'Sem nome'}>{manager.name || 'Sem nome'}</h3>
                            <p className="text-sm text-muted-foreground truncate" title={manager.email}>
                              {manager.email}
                            </p>
                            {manager.phone && (
                              <p className="text-xs text-muted-foreground truncate" title={manager.phone}>
                                📞 {manager.phone}
                              </p>
                            )}
                            {manager.document && (
                              <p className="text-xs text-muted-foreground truncate" title={manager.document}>
                                📄 {manager.document}
                              </p>
                            )}
                            {manager.address && (
                              <p className="text-xs text-muted-foreground truncate mt-1" title={`${manager.address}${manager.city ? `, ${manager.city}` : ''}${manager.state ? ` - ${manager.state}` : ''}`}>
                                📍 {manager.address}
                                {manager.city && `, ${manager.city}`}
                                {manager.state && ` - ${manager.state}`}
                              </p>
                          ...
                          <div className="flex items-center justify-between mt-2 gap-2 flex-shrink-0">
                            ...
                            <DropdownMenu>
                              ...
                              <DropdownMenuContent align="end">
                                ...
                                {canDeleteUsers && (
                                  <DropdownMenuItem onClick={() => handleDeleteManager(manager)} className="text-red-600 focus:text-red-700">
        ...
        <AlertDialog open={!!managerToDelete} onOpenChange={() => setManagerToDelete(null)}>
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                Tem certeza que deseja excluir o gerente <b>{managerToDelete?.name}</b>? Esta ação não poderá ser desfeita.
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
