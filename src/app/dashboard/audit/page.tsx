...
export default function AuditLogsPage() {
    ...
    return (
      <div className="flex items-center justify-center min-h-[400px]">
        <div className="text-center">
          ...
          <p className="text-muted-foreground">Você não tem permissão para visualizar logs de auditoria.</p>
        </div>
      </div>
    )
  }
  ...
  const getEventBadgeColor = (event: string) => {
    if (event.includes('CREATE') || event.includes('create')) return 'bg-green-500'
    if (event.includes('UPDATE') || event.includes('update')) return 'bg-blue-500'
    if (event.includes('DELETE') || event.includes('delete')) return 'bg-red-500'
    return 'bg-gray-500'
  }
  ...
  const exportToCSV = () => {
    ...
    const headers = ['Data', 'Usuário', 'Evento', 'Entidade', 'ID da Entidade', 'Endereço IP', 'User Agent']
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-sm sm:text-base text-muted-foreground mt-1">
            Visualize atividades e alterações do sistema
          </p>
        </div>
        <Button 
          onClick={exportToCSV}
          ...
          <CardContent>
            ...
            <div className="flex justify-between items-center mt-6 pt-4 border-t">
              <p className="text-sm text-muted-foreground">
                Página {auditData.page} de {Math.ceil(auditData.total / auditData.pageSize)}
              </p>
