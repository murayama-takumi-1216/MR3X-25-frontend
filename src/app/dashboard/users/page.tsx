...
type UserItem = { id: string; name: string | null; email: string; role: string; status: string; plan?: string }
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
        ...
        <select value={plan} onChange={e => setPlan(e.target.value)} className="border rounded-md px-3 py-2">
          ...
          {['FREE','ESSENCIAL','PROFISSIONAL','PREMIUM'].map(p => <option key={p} value={p}>{p}</option>)}
        </select>
      </div>
          ...
          {items.map((u: any) => (
            <div key={u.id} className="border border-border rounded-lg p-4 space-y-3 bg-card shadow-sm min-w-0 overflow-hidden">
              <div className="min-w-0">
                <h3 className="text-base font-semibold leading-tight truncate" title={u.name || 'Sem nome'}>{u.name || 'Sem nome'}</h3>
                <p className="text-xs text-muted-foreground truncate">
                  Criado em {u.createdAt ? new Date(u.createdAt).toLocaleDateString('pt-BR') : 'data desconhecida'}
                </p>
              </div>
              <div className="text-sm min-w-0">
                <span className="font-medium text-muted-foreground">Email:</span> <span className="truncate block" title={u.email}>{u.email}</span>
              </div>
