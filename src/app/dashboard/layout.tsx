...
import { Home, Building2, Users, FileText, DollarSign, MessageSquare, Bell, LogOut, Menu, X, BarChart3, User, Shield, Building, Briefcase, UserCheck, Scale, Handshake, UserCog, ShieldCheck, Settings, FileDown, Crown, Package, Mail, Wrench, Receipt, Key } from 'lucide-react'
...
const baseNavigation = [
  ...
  // Shared pages (non-role restricted)
...
export default function DashboardLayout({
  ...
  const navigation = baseNavigation.filter(item => {
    ...
    // They see aggregated data through Dashboard and Reports, and use Communications for messaging
    if (user?.role === 'CEO') {
      const excludeForCEO = [
        '/dashboard/properties',  // Property management - CEO sees stats in dashboard
        '/dashboard/contracts',  // Contract management - CEO sees stats in dashboard
        '/dashboard/payments',   // Payment management - CEO sees stats in dashboard
        '/dashboard/notifications', // Individual notifications - CEO uses Communications for platform-wide messaging
        '/dashboard/brokers',    // Broker management - CEO uses main Users page
        '/dashboard/tenants',    // Tenant management - CEO uses main Users page
        '/dashboard/owners',      // Owner management - CEO uses main Users page
      ]
      if (excludeForCEO.includes(item.href)) return false
    }

    // AGENCY_ADMIN doesn't need tenants, brokers, or owners pages
    // They manage managers, properties, contracts, and payments within their agency
    if (user?.role === 'AGENCY_ADMIN') {
      const excludeForAgencyAdmin = [
        '/dashboard/agencies',   // Agencies - platform list, AGENCY_ADMIN only manages their own agency
        '/dashboard/users',      // Global users management page is hidden for agency admins
      ]
    ...
    if (user?.role === 'INDEPENDENT_OWNER') {
      const excludeForIndependentOwner = [
        '/dashboard/brokers',    // No brokers for independent owners
        '/dashboard/owners',      // They are the owner
        '/dashboard/agencies',    // Not linked to agencies
        '/dashboard/managers',    // No managers
        '/dashboard/agency-admin', // Not agency admin
        '/dashboard/agency-split-config', // Different split config (should be in settings)
        '/dashboard/agency-plan-config', // Not linked to agency plans
        '/dashboard/users',      // Users page - independent owners manage tenants via tenants page
      ]
    ...
    if (user?.role === 'PROPRIETARIO') {
      const excludeForProprietario = [
        '/dashboard/brokers',    // Brokers are managed by agency
        '/dashboard/owners',      // They are the owner
        '/dashboard/agencies',    // Not linked to agencies (or different context)
        '/dashboard/managers',    // No managers
        '/dashboard/agency-admin', // Not agency admin
        '/dashboard/agency-split-config', // Split handled by agency
        '/dashboard/agency-plan-config', // Plans handled by agency
      ]
