...
export default function TenantsPage() {
    ...
    return (
      <div className="flex items-center justify-center min-h-[400px]">
        <div className="text-center">
          ...
          <p className="text-muted-foreground">Você não tem permissão para visualizar inquilinos.</p>
        </div>
      </div>
    )
  }
  ...
  const createTenantMutation = useMutation({
    mutationFn: (data: any) => usersAPI.createTenant({
      ...data,
    ...
    onError: (error: any) => {
      ...
      if (error.message) {
        ...
        if (message.includes('already exists') || message.includes('user already exists') || message.includes('email already exists')) {
          ...
          errorMessage = 'Sem permissão para criar inquilino.'
        } else {
          // Use the actual error message if it's in Portuguese or a specific error
  ...
  const updateTenantMutation = useMutation({
    ...
    onError: (error: any) => {
      ...
      // Extract specific error message
      let errorMessage = 'Erro ao atualizar inquilino'
      ...
      if (error.message) {
        ...
        if (message.includes('already exists') || message.includes('email already exists')) {
          ...
          errorMessage = 'Sem permissão para atualizar inquilino.'
        } else {
          errorMessage = error.message
        }
      }
  ...
  const deleteTenantMutation = useMutation({
    ...
    onError: (error: any) => {
      ...
      const errorMessage = error?.message || error?.data?.message || 'Erro ao excluir inquilino'
      toast.error(errorMessage)
    },
  })
  ...
  const handleCreateTenant = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const docResult = enable2026 ? validateDocument2026(newTenant.document) : validateDocument(newTenant.document)
      ...
      // Default password for tenant first login
  ...
  const handleUpdateTenant = async (e: React.FormEvent) => {
    ...
    try {
      ...
      updateTenantMutation.mutate({ id: selectedTenant.id, data: tenantToSend })
    } finally {
      setUpdating(false)
    }
  }
  ...
  const handleNewTenantCEPData = (data: any) => {
  ...
  const handleEditTenantCEPData = (data: any) => {
  ...
  const handleWhatsAppNotification = (tenant: any) => {
    closeAllModals()
    setSelectedTenant(tenant)
    setWhatsappMessage(`Olá ${tenant.name}! Este é um lembrete sobre seu aluguel.`)
      ...
      <div className="space-y-6">
          ...
          <div className="flex items-center gap-2 w-full sm:w-auto">
            ...
            {showCreateTenantButton && (
              <Tooltip>
                ...
                <TooltipContent>Cadastrar Inquilino</TooltipContent>
              </Tooltip>
            )}
          </div>
        </div>
                  ...
                  <div key={tenant.id} className="border-b border-border last:border-b-0 p-4">
                    <div className="flex items-start justify-between mb-3 min-w-0 gap-2">
                      <div className="flex-1 min-w-0">
                        <h3 className="font-semibold text-lg truncate" title={tenant.name || 'Sem nome'}>{tenant.name || 'Sem nome'}</h3>
                        <p className="text-sm text-muted-foreground truncate" title={tenant.email || '-'}>{tenant.email || '-'}</p>
                      </div>
                    ...
                    <div className="space-y-2 mb-4 min-w-0">
                      {tenant.phone && (
                        <div className="flex items-center text-sm text-muted-foreground min-w-0">
                          ...
                          <span className="truncate" title={tenant.phone}>{tenant.phone}</span>
                        </div>
                      )}
                      {tenant.address && (
                        <div className="flex items-start text-sm text-muted-foreground min-w-0">
                          ...
                          <span className="truncate" title={`${tenant.address}${tenant.city ? `, ${tenant.city}` : ''}${tenant.state ? ` - ${tenant.state}` : ''}`}>
                            {tenant.address}
                            {tenant.city && `, ${tenant.city}`}
                            {tenant.state && ` - ${tenant.state}`}
                          </span>
        ...
        <Dialog open={showCreateModal} onOpenChange={setShowCreateModal}>
          <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto mx-4 sm:mx-0">
            <DialogHeader>
              <DialogTitle>Cadastrar Locatário</DialogTitle>
            </DialogHeader>
        ...
        <AlertDialog open={!!tenantToDelete} onOpenChange={() => setTenantToDelete(null)}>
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                Tem certeza que deseja excluir o inquilino <b>{tenantToDelete?.name}</b>? Esta ação não poderá ser desfeita.
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
